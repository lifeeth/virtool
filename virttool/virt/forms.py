...
class DiskForm(forms.ModelForm):
    ...
    readonly = forms.BooleanField(label=_('Read only ?'), initial=True)
    shareable = forms.BooleanField(label=_('Shareable ?'), initial=True, \
...
class InterfaceForm(forms.ModelForm):
    ...
    interface_type = forms.CharField(label=_('Type'), max_length=5, widget=forms.Select(choices=virt.models.INTERFACE_TYPES))
    ...
    mac = extrafields.MACAddressField()
